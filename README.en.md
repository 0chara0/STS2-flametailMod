...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/flametail/wiki/Home) | [English Home](https://github.com/alkaid616/flametail/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `flametail`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `flametail.json` and update `id`, `name`, `author`, and `description`.
3. Edit `flametailCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `flametail/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
| Current RitsuLib version | `0.4.54` |
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `flametail.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `flametail.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\flametail.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/flametail` directory.
...
```text
flametail/
├── flametailCode/   # C# source
├── flametail/       # Godot resources, localization, and placeholder scenes
├── flametail.csproj
├── flametail.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://flametail/...` is the Godot/PCK resource path, mapping to the repository resource directory `flametail/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `flametailCharacter` |
| Expected ID | `FLAMETAIL_CHARACTER_FLAMETAIL_CHARACTER` |
| Starter deck | 4 × `flametailStrike`, 4 × `flametailDefend`, 1 × `flametailRelic` |
...
|---|---|---|
| `flametailStrike` (attack) | character card pool | `FLAMETAIL_CARD_FLAMETAIL_STRIKE` |
| `flametailDefend` (skill) | character card pool | `FLAMETAIL_CARD_FLAMETAIL_DEFEND` |
| `flametailRelic` | `flametailRelicPool` | `FLAMETAIL_RELIC_FLAMETAIL_RELIC` |

### Static placeholder assets

**Images** (`res://flametail/images/...`):

- `cards/flametailStrike.png`, `cards/flametailDefend.png`: sample card art.
- `relics/flametailRelic.png`: sample relic icon.
- `characters/flametail_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://flametail/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `flametail_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `flametail_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `flametail_merchant.tscn` | Merchant | — |
| `flametail_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `flametail_character_select_bg.tscn` | Character select background | — |
...
`flametail.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
